	...
	try {
		...
		let [rawPriceHistory, rawTransactions] = await Promise.all([
			db
				...
				.from(priceHistory)
				.where(and(eq(priceHistory.coinId, coinData.id), gte(priceHistory.timestamp, sinceDate)))
			...
			db
				...
				.from(transaction)
				.where(and(eq(transaction.coinId, coinData.id), gte(transaction.timestamp, sinceDate)))
			...
			if (latestPrice) {
				...
				[rawPriceHistory, rawTransactions] = await Promise.all([
					db
						...
						.from(priceHistory)
						.where(
							and(eq(priceHistory.coinId, coinData.id), gte(priceHistory.timestamp, sinceDate))
						)
						.orderBy(desc(priceHistory.timestamp))
						.limit(5000),
