<img style="width: 128px; height: 128px" src="website/static/dingoplay.png" /><h1 style="font-size: 48px"><a href="https://dingoplay.dpdns.org">dingoplay.dpdns.org</a> - the fake crypto simulator.</h1>
...
2. **Set up Redis**
   ...
   ```bash
   docker run -d --name dingoplay-redis -p 6379:6379 -v dingoplay_redisdata:/data --restart unless-stopped redis:8-alpine redis-server --save 60 1
   ```
...
3. **Set up PostgreSQL**
   ...
   - **Run Postgres locally with Docker:**
     ```bash
     docker run -d --name dingoplay-postgres -e POSTGRES_USER=dingouser -e POSTGRES_PASSWORD=dingopass -e POSTGRES_DB=dingoplay -p 5432:5432 -v dingoplay_pgdata:/var/lib/postgresql/data --restart unless-stopped pgvector/pgvector:pg16
     ```
   ...
   ```bash
   docker exec -it dingoplay-postgres psql -d dingoplay -U dingouser
   ```
...
5. **Configure Environment Variables**
   ...
   ```ini
   # --- Database ---
   DATABASE_URL=postgres://dingouser:dingopass@localhost:5432/dingoplay   # PostgreSQL connection string
   POSTGRES_USER=dingouser      # PostgreSQL username (should match Docker config)
   POSTGRES_PASSWORD=dingopass  # PostgreSQL password (should match Docker config)
   POSTGRES_DB=dingoplay       # PostgreSQL database name (should match Docker config)
