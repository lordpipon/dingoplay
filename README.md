...
[Privacy Policy](https://dingoplay.dpdns.org/legal/privacy) | [Terms of Service](https://dingoplay.dpdns.org/legal/terms) | [License](LICENSE) | [YouTube video](https://www.youtube.com/watch?v=nRUkvPMphRc)
...
1. **Clone the repository**
   ...
   ```bash
   git clone https://github.com/lordpipon/dingoplay.git
   cd Dingoplay
   ```
...
3. **Set up PostgreSQL**
   ...
   - **Run Postgres locally with Docker:**
     ```bash
     docker run -d --name dingoplay-postgres -e POSTGRES_USER=pguser -e POSTGRES_PASSWORD=pgpass -e POSTGRES_DB=dingoplay -p 5432:5432 -v dingoplay_pgdata:/var/lib/postgresql/data --restart unless-stopped pgvector/pgvector:pg16
     ```
   ...
   ```bash
   docker exec -it dingoplay-postgres psql -d dingoplay -U pguser
   ```
...
5. **Configure Environment Variables**
   ...
   ```ini
   # --- Database ---
   DATABASE_URL=postgres://pguser:pgpass@localhost:5432/dingoplay   # PostgreSQL connection string
